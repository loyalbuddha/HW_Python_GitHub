...
with open('text.txt', 'r') as text:
    ...
    for i in range(0, len(content)):
        if int(content[i].split()[2]) >= int(content[i].split()[3]) :
            summ = summ + int(content[i].split()[2])
    avg_profit = summ / len(content)
    cost_dict = {content[i].split()[0] : (int(content[i].split()[2]) - int(content[i].split()[3])) for i in range(0, len(content))}
    cost_list = [cost_dict, {'avg_profit':avg_profit}]
    print(cost_list)

with open('text.json', 'w') as text:
    json.dump(cost_list, text)